...
define(['handlebars','reg','users','profile','messages'],function(handlebars,reg,users,profile,messages){
	...
	reg.regButton.addEventListener('click',function(e){
		...
		// Проверяем чтобы были заполнены поля 
		if(!reg.regName.value.trim()) {
			reg.regError.innerHTML = "Не заполнено поле Имя";
		...
		connection.onmessage = function(e) {
			...
			if (answerType == "error") {
				...
				lastMessages.forEach(function(item){
					...
					newMessage.classList.add('right__item','wrapper');
				...
				xhr.open('GET', 'http://localhost:8080/photos/'+answerObj['user']['login'], true);
				...
				xhr.onreadystatechange = function() {
					if (xhr.readyState != 4) {return};
						...
						if (xhr.status = 200) {
							...
							userProfileImg = "http://localhost:8080/photos/"+answerObj['user']['login']+"?"+Date.now();
